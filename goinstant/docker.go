...
func RunDirectDockerCommand(startupCommands []string) error {
	...
	if deployCommand == "init" {
		...
		_, err = runCommand(deployEnvironment, nil, commandSlice...)
		if err != nil {
			return err
		}
	}
	...
	for _, c := range customPackagePaths {
		fmt.Print("- " + c)
		err = mountCustomPackage(deployEnvironment, c)
		if err != nil {
			return err
		}
	}
...
func mountCustomPackage(deployEnvironment string, pathToPackage string) error {

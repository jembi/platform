...
// Service should have Running status for more than 30 seconds
async function checkServiceStatus(serviceName) {
  let serviceStatuses = '';
  let timeNow = Date.now();
  let timePassedWithRunningStatus = 0;
  while (!serviceStatuses.includes('Running') || timePassedWithRunningStatus < 30 * 1000) {
    const { stdout, stderr } = await execPromise(
    ...
    } else if (Date.now() - timeNow > 30 * 1000) {

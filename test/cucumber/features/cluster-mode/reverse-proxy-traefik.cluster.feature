Feature: Reverse Proxy Traefik?
  ...
  Scenario: Init Reverse Proxy Traefik
    Given I use parameters "package init -n=reverse-proxy-traefik --env-file=.env.cluster"
  ...
  Scenario: Destroy Reverse Proxy Traefik
    Given I use parameters "package destroy -n=reverse-proxy-traefik --env-file=.env.cluster"
    When I launch the platform with params

Feature: Openfn?
  ...
  Scenario: Init Openfn
    Given I use parameters "package init -n=openfn --dev --env-file=.env.cluster"
    ...
    And The service "openfn-postgres-2" should be started with 1 replica
    And The service "openfn-postgres-2" should be connected to the networks
      | openfn_postgres_public | pg_backup |
    And The service "openfn-postgres-3" should be started with 1 replica
    And The service "openfn-postgres-3" should be connected to the networks
      | openfn_postgres_public | pg_backup |
    ...
    And There should be 4 volume

  Scenario: Destroy Openfn
    Given I use parameters "package destroy -n=openfn --only --dev --env-file=.env.cluster"
    When I launch the platform with params

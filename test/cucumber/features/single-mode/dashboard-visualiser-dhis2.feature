Feature: Dashboard Visualiser DHIS12
  ...
  Scenario: Init DHIS2
    Given I use parameters "package init -n=dashboard-visualiser-dhis2 --dev --env-file=.env.local"
  ...
  Scenario: Destroy DHIS2
    Given I use parameters "package destroy -n=dashboard-visualiser-dhis2 --only --dev --env-file=.env.local"
    When I launch the platform with params

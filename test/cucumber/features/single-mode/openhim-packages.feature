Feature: Openhim and its dependent packages?
    ...
    Scenario: Init Openhim Mapping Mediator
        Given I use parameters "init openhim-mapping-mediator --only --dev --env-file=.env.local"
        When I launch the platform with params
    ...
    Scenario: Destroy Openhim and its dependent packages
        Given I use parameters "destroy interoperability-layer-openhim client-registry-jempi openhim-mapping-mediator --only --dev --env-file=.env.local"
        When I launch the platform with params
